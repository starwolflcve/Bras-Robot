...
PIN_J1_SW = 5  # [CORRIGÉ] Bouton J1 sur GPIO 5 (numérique) → Reset base + bras 1
 
PIN_J2_X = 4   # ADC4 - Joystick 2 axe X → Bras 2
PIN_J2_Y = 2   # [CORRIGÉ] ADC2 (sur GPIO 2) - Joystick 2 axe Y → Pince
...
ANGLE_CHANGE_MIN = 2     # Changement minimum (°) pour bouger le servo (tu peux le supprimer ou le garder)
NB_SMOOTH = 10           # Nombre de lectures pour lissage
NB_CALIB  = 20           # Nombre de lectures pour calibration repos

MAX_STEP_ANGLE = 2.0     # NOUVEAU : Vitesse max (degrés à ajouter par cycle de 20ms)
...
def adc_to_step(valeur, repos):
    """
    Convertit une valeur ADC en un pas (incrément) selon l'inclinaison du joystick.
    Le repos retourne 0 (aucun mouvement).
    """
    delta = valeur - repos
    if abs(delta) <= DEAD_ZONE:
        return 0.0  # Dans la zone morte, on ne bouge pas
    
    if delta > 0:
        # Poussé dans un sens
        return MAX_STEP_ANGLE * (delta - DEAD_ZONE) / (ADC_MAX - repos - DEAD_ZONE)
    else:
        # Poussé dans l'autre sens
...
def main():
    ...
    # [MODIFIÉ] Angles courants mémorisés en float pour plus de fluidité
    angle_bras1 = 0.0
    angle_bras2 = 0.0
    angle_pince = 0.0
    ...
    while True:
        ...
        # ---- BRAS 1 (J1 axe Y) ----
        step1 = adc_to_step(val_y1, rest_y1)
        if step1 != 0:
            angle_bras1 = max(0, min(180, angle_bras1 + step1)) # On ajoute le pas tout en bloquant entre 0 et 180
            set_angle(pwm_bras1, int(angle_bras1))
 
        # ---- BRAS 2 (J2 axe X) ----
        step2 = adc_to_step(val_x2, rest_x2)
        if step2 != 0:
            angle_bras2 = max(0, min(180, angle_bras2 + step2))
            set_angle(pwm_bras2, int(angle_bras2))
 
        # ---- PINCE (J2 axe Y) ----
        step3 = adc_to_step(val_y2, rest_y2)
        if step3 != 0:
            angle_pince = max(0, min(180, angle_pince + step3))
